...
> [!IMPORTANT]\
> Please disregard a lot of what is in the text below. After someone in the Slack (@Toby) introduced me to the world of AliExpress, I completely redid the BOM. It is now $119.89 (yes, that is USD lol), so way under the limit. Thanks a lot, dude :).

The BOM can be found in [`/bom.csv`](bom.csv). Since this is my first ever hardware project, I decided to go for some products that had easier to find footprints/symbols in KiCad for. With hindsight, spending the amount of money I now have to on a nice! nano v2 or nice! view (which I landed on because I chose the nice! nano v2) isn't really necessary. So, in future projects I'll choose other, cheaper components - but for this one I've already designed the PCB etc, and was very new to this whole world of design, and so decided to go with components that I was more confident with.
...
---
<sub>Thanks to [Hackclub](https://hackclub.com) for such an amazing opportunity - this project was made by [@Atreyu-D10](https://github.com/Atreyu-D10) with 🩷, under the [MIT License](LICENSE).</sub>
